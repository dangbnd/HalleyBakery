...
export function Hero({ products = [], interval = 2000, fbUrls = [], onBannerClick }) {
  ...
  // B5: clamp i vào range hợp lệ, guard khi view thay đổi phía dưới
  const safeI = Math.max(0, Math.min(i, view.length - 1));
                  ...
                  {view.map((s, idx) => (
                    <div key={`${s.id}-${idx}`} className="w-full h-full basis-full shrink-0" style={{ contain: "layout paint" }}>
                      <button type="button"
                        ...
                        <img src={cdn(s.src, { w: 960, q: 70 })} alt={s.alt} className="w-full h-full object-cover cursor-zoom-in" draggable={false} />
                      </button>
                    </div>
                  ))}
                </div>
